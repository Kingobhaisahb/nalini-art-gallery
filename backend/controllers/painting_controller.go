...
func (p *PaintingController) GetSoldPaintings(c *gin.Context) {
...
func (p *PaintingController) SearchPaintings(c *gin.Context) {
	...
	paintings, err := p.PaintingService.SearchPaintings(query)

	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": "Failed to search paintings",
		})
		return
	}
...
func (p *PaintingController) GetNewestPaintings(c *gin.Context) {

	paintings, err := p.PaintingService.GetNewestPaintings()

	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": "Failed to fetch newest paintings",
		})
		return
	}
...
func (p *PaintingController) GetAdminPaintings(c *gin.Context) {
	...
	responses := make([]dto.AdminPaintingResponse, 0, len(paintings))

	for _, painting := range paintings {

		response := dto.AdminPaintingResponse{
			ID:              painting.ID,
			Title:           painting.Title,
			Price:           painting.Price,
			Description:     painting.Description,
